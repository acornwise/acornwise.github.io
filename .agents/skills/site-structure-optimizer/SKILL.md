...
Uses ~~web crawler, ~~SEO tool, and ~~analytics when connected; otherwise asks the user for site type, page inventory or sitemap, key page URLs, content categories, and existing URLs. Every step works manually from a provided page list. See [CONNECTORS.md](../../references/CONNECTORS.md) and [SECURITY.md §Scraping Boundaries](../../../SECURITY.md).

**Zero-dependency local helper** (no tool needed):
- Architecture / seed inventory: `python3 ".agents/scripts/connectors/crawl.py" <url>` returns the live page list and link graph.
- Linking metrics: `python3 ".agents/scripts/connectors/crawl.py" <url> | python3 ".agents/scripts/connectors/linkgraph.py" -` computes orphans, click-depth, and internal PageRank.
