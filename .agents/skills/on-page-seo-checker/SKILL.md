...
Use ~~web crawler, ~~SEO tool, and ~~search console when connected; otherwise ask for page URL/HTML, target keywords, and competitor URLs. See [CONNECTORS.md](../../references/CONNECTORS.md) and [SECURITY.md §Scraping Boundaries](../../../SECURITY.md).

**Zero-dependency local helpers** (no tool needed): `python3 ".agents/scripts/connectors/onpage.py" <url>` (title/meta/headings/canonical/JSON-LD/redirects) and `schema_lint.py <url>` (structured-data validation). See [scripts/connectors/README.md](../../scripts/connectors/README.md).

**JS-rendering fallback (keyless)**: if `onpage.py` returns a near-empty body (client-side rendering), `python3 ".agents/scripts/connectors/firecrawl.py" scrape <url> --formats markdown,html` supplies the rendered content so heading/keyword/word-count checks audit what crawlers actually index, not the unhydrated shell. robots.txt is pre-flighted locally (Disallow refused); `--own-site` for your own staging hosts. Firecrawl keyless free tier.
