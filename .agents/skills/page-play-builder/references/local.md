...
Use `~~local listings` and `~~search console` when connected; otherwise work Tier-1 manually — the user's own GBP dashboard export and a manual NAP/citation check against each directory. No paid audit tool is required. See [CONNECTORS.md](../../../references/CONNECTORS.md).

## Handoff

**Primary next skill**: [on-page-seo-checker](../../../tune/on-page-seo-checker/SKILL.md) — audit the location/service-area pages once drafted.
